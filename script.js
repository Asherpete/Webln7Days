document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    burgerMenu.addEventListener("click", function () {
      // Toggle class for opening/closing menu
  ...
  // Touch-friendly interaction for clickable boxes
  document.querySelectorAll(".j-box, .box").forEach((element) => {
...
const lessons = {
  1: {
    ...
    subtitle:
      "Get familiar with web development basics and set up your environment.",
    content: `
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".openModalBtn").forEach((btn) => {
    ...
    btn.addEventListener("click", () => {
      ...
      markCompleteBtn.textContent = isCompleted
        ? "✓ Completed"
        : "Mark as Complete";
  ...
  markCompleteBtn.addEventListener("click", () => {
    ...
    if (!completedLessons.includes(lessonNum)) {
      ...
      const lessonBtn = document.querySelector(
        `.openModalBtn[data-lesson="${lessonNum}"]`
      );
      if (lessonBtn) {
  ...
  goToLessonBtn.addEventListener("click", () => {
    alert(
      `Redirecting to full lesson ${modalLessonNumber.textContent} page...`
    );
  });
});
...
function loadComments() {
  const savedComments =
    JSON.parse(localStorage.getItem("website-comments")) || [];
